...
export const AvatarConfig: React.FC<AvatarConfigProps> = ({
  ...
  return (
    <div
      className="relative flex flex-col gap-4 w-full py-8 max-h-full overflow-y-auto px-4"
      style={{ maxWidth: 600 }}
    >
      <Field label="Lexware Office API-Schlüssel">
        <Input
          className="!bg-gray-100"
          placeholder="Lexware Office API-Schlüssel eingeben..."
      ...
      {selectedAvatar?.isCustom && (
        <Field label="Benutzerdefinierte Agent ID">
          <Input
            className="!bg-gray-100"
            placeholder="Benutzerdefinierte Agent ID eingeben"
      ...
      {showMore && (
        <>
          <h1 className="text-primary w-full text-center mt-5 text-sm font-semibold">
            Voice Settings
          </h1>
          <Field label="Benutzerdefinierte Voice ID">
            <Input
              className="!bg-gray-100"
              placeholder="Benutzerdefinierte Voice ID eingeben"
      ...
      <button
        className="text-secondary text-sm cursor-pointer w-full text-center bg-transparent underline"
        onClick={() => setShowMore(!showMore)}
      >
        {showMore ? "Weniger anzeigen" : "Erweiterte Einstellungen"}
      </button>
    </div>
  );
};
